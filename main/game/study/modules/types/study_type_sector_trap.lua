...
function M.start(self)
	...
	msg.post("/loader_gui", "set_content", {
		...
		items = {
			{
				dialog_id = "_study_sector_trap_1", 
				text = lang_core.get_text(self, "_study_sector_trap_1"),
				bg = false
			},
		}
	})
...
function M.on_message(self, message_id, message, sender)
	if message and message.type == "close_character_dialog" and message.value.dialog_id == "_study_sector_trap_1" then
		msg.post("/loader_gui", "set_status", {
			id = "game_wheel",
			type = "study",
			value = {
