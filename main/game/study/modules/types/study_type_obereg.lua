...
function M.start(self)
	...
	msg.post("/loader_gui", "set_content", {
		...
		items = {
			{
				dialog_id = "_study_obereg_1", 
				text = lang_core.get_text(self, "_study_obereg_1")
			},
		}
	})

	-- Добавляю новый оберег
	local player_id = core_layouts.get_data().data.player.player_id
	local artifact_id = "try_1"
	local count = 1
...
function M.on_message(self, message_id, message, sender)
	if message and message.type == "close_character_dialog" and message.value.dialog_id == "_study_obereg_1" then
				...
				timeline = {
					{
						type = "touch",
						gui_object_end = {
							id_comonent = "modal_obereg", node_name = "btn" -- ключи в storage_gui.positions
						},
					}
				}
			})
	...
	elseif message and message.id == "close_gui" and message.component_id == "modal_obereg" then
