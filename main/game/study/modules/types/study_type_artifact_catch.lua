...
function M.start(self)
	...
	msg.post("/loader_gui", "set_content", {
		...
		items = {
			{
				dialog_id = "_study_artifact_catch_1", 
				text = lang_core.get_text(self, "_study_artifact_catch_1"),
			},
		}
	})
...
function M.on_message(self, message_id, message, sender)
	if message and message.type == "close_character_dialog" and (message.value.dialog_id == "_study_artifact_catch_1") then
		-- Про прицел
				...
				timeline = {
					{
						type = "touch",
						gui_object_end = {
							id_comonent = "game_hud_buff_horisontal", node_name = "buff_2" -- ключи в storage_gui.positions
						},
					}
				}
			})
