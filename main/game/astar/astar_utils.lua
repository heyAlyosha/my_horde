...
-- Takes a zero based 2d tile coordinate, returns a 2d screen coordinate
 function utils:coords_to_screen(x, y)
    local scrx = astar_settings.tile_size * x + astar_settings.tile_size / 2
    local scry = astar_settings.tile_size * y + astar_settings.tile_size / 2
    return scrx, scry
end

-- Takes a 2d screen coordinate, returns a zero based 2d tile coordinate
 function utils:screen_to_coords(x, y)
    return floor(x / astar_settings.tile_size), floor(y / astar_settings.tile_size)
end

--
 function utils:snap_screen_to_tile_center(x, y)
    local snapped_x = floor(x / astar_settings.tile_size) * astar_settings.tile_size + astar_settings.tile_size / 2
    local snapped_y = floor(y / astar_settings.tile_size) * astar_settings.tile_size + astar_settings.tile_size / 2
