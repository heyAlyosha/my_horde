...
function M.add_target(self, url_target)
	...
	for k, item in pairs(target.targets) do
		if item.is_move then
			possible_targets[#possible_targets+1] = {
				...
				sort = item.count_object * 1000 + vmath.length(item.position - position)
			}
		end
	end
	...
	storage_game.go_targets[key_target].targets[possible_targets[1].id].count_object = storage_game.go_targets[key_target].targets[possible_targets[1].id].count_object + 1
end
